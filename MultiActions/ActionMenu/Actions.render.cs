...
namespace MultiActions.ActionMenu
{
    public static class MaActionsRender
    {
        ...
        private static void RenderActions(MaHigherActions pos)
        {
            // Lets get all MaActionsList with the "Main" category
            var actions = MaActionsList.Where(x => x.HigherAction == pos).ToList();
            ...
            foreach (var action in actions)
            {
                ...
                {
                    // Add submenu
                    CustomSubMenu.AddSubMenu(action.Name, () =>
                    {
                        RenderActions(pos, action.subAction);
                    });
                }
                else if (action.hasSubMenus)
                {
                    // Add submenu
                    CustomSubMenu.AddSubMenu(action.Name, () =>
                    {
                        RenderActions(pos, action.subActions);
                    });
                }
                else
                {
        ...
        private static void RenderActions(MaHigherActions pos, MaAction action)
        {
            // Check if subAction
            if (action.hasSubMenu)
            {
        ...
        private static void RenderActions(MaHigherActions pos, MaAction[] actions)
        {
            foreach(var action in actions)
            {
                // Check if subAction
